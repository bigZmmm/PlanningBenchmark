(define (domain grid)
   ...
   (:types num )
   (:constants zero one two three four five six seven eight nine ten eleven twelve  -num)
   (:predicates (x ?i - num) (y ?i - num) (canmove))

   (:action can_move
      :parameters()
      :effect (and
		(when(and (x ten) (y four)) (not (canmove)))
		(when(not (and (x ten) (y four))) (canmove))
   ...
   (:action east
      ...
      :effect (and
		...
		(when (and (not (y four)) (x nine) (canmove)) (and (x ten) (not (x nine))))
