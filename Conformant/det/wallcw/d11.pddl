(define (domain grid)
   ...
   (:types num )
   (:constants zero one two three four five six seven eight nine ten  -num)
   (:predicates (x ?i - num) (y ?i - num) (canmove))

   (:action can_move
      :parameters()
      :effect (and
		(when(and (x nine) (y three)) (not (canmove)))
		(when(not (and (x nine) (y three))) (canmove))
   ...
   (:action east
      ...
      :effect (and
		...
		(when (and (not (y three)) (x eight) (canmove)) (and (x nine) (not (x eight))))
