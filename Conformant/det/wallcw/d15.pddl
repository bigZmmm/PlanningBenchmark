(define (domain grid)
   ...
   (:types num )
   (:constants zero one two three four five six seven eight nine ten eleven twelve thirteen fourteen  -num)
   (:predicates (x ?i - num) (y ?i - num) (canmove))

   (:action can_move
      :parameters()
      :effect (and
		(when(and (x twelve) (y five)) (not (canmove)))
		(when(not (and (x twelve) (y five))) (canmove))
   ...
   (:action east
      ...
      :effect (and
		...
		(when (and (not (y five)) (x eleven) (canmove)) (and (x twelve) (not (x eleven))))
