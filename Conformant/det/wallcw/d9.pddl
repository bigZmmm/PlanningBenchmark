(define (domain grid)
   ...
   (:types num )
   (:constants zero one two three four five six seven eight  -num)
   (:predicates (x ?i - num) (y ?i - num) (canmove))

   (:action can_move
      :parameters()
      :effect (and
		(when(and (x seven) (y three)) (not (canmove)))
		(when(not (and (x seven) (y three))) (canmove))
   ...
   (:action east
      ...
      :effect (and
		...
		(when (and (not (y three)) (x six) (canmove)) (and (x seven) (not (x six))))
