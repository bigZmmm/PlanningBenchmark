(define (domain grid)
   ...
   (:types num )
   (:constants zero one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen  -num)
   (:predicates (x ?i - num) (y ?i - num) (canmove))

   (:action can_move
      :parameters()
      :effect (and
		(when(and (x fourteen) (y five)) (not (canmove)))
		(when(not (and (x fourteen) (y five))) (canmove))
   ...
   (:action east
      ...
      :effect (and
		...
		(when (and (not (y five)) (x thirteen) (canmove)) (and (x fourteen) (not (x thirteen))))
