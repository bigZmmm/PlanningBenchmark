...
(define (domain d-ring-new-12)
  (:requirements :typing :equality)
  (:types window)
  (:constants  w1  w2  w3  w4  w5  w6  w7  w8  w9  w10  w11  w12  - window)
  (:predicates
     (pos ?w - window)
     (closed ?w - window)
     (open ?w - window)
  ...
  (:action fwd
     :effect
       (and 
            ...
            (when (pos w12) (and (pos w1) (not (pos w12))))  
       )
  )
  (:action bwd
     :effect
       (and 
            (when (pos w1) (and (pos w12) (not (pos w1))))  
  ...
  (:action lock
     :effect
       (and 
            ...
            (when (and (pos w10) (closed w10) (not (locker_damaged w10))) (locked w10))  
            (when (and (pos w11) (closed w11) (not (locker_damaged w11))) (locked w11))  
            (when (and (pos w12) (closed w12) (not (locker_damaged w12))) (locked w12))  
       )
  )
)
