  ...
  (:action unlock
     ...
     :effect
       (and 
            ...
            (when (and ;(has-key) 
                       (pos w26)) (not (locked w26)))  
            (when (and ;(has-key) 
                       (pos w27)) (not (locked w27)))  
            (when (and ;(has-key) 
                       (pos w28)) (not (locked w28)))  
            (when (and ;(has-key) 
                       (pos w29)) (not (locked w29)))  
            (when (and ;(has-key) 
                       (pos w30)) (not (locked w30)))  
            (when (and (not (locked w1)) (not (pos w1))) 
                  (oneof (closed w1) (not (closed w1))))
  ...
  (:action pick-up
     :effect
       (and 
            ...
            (when (and (pos w26) (key-at w26)) (has-key))  
            (when (and (pos w27) (key-at w27)) (has-key))  
            (when (and (pos w28) (key-at w28)) (has-key))  
            (when (and (pos w29) (key-at w29)) (has-key))  
            (when (and (pos w30) (key-at w30)) (has-key))  
            (when (and (not (locked w1)) (not (pos w1))) 
                  (oneof (closed w1) (not (closed w1))))
  ...
  (:action put-down
     :effect
       (and
            ...
            (when (and (has-key) (pos w26)) (key-at w26))  
            (when (and (has-key) (pos w27)) (key-at w27))  
            (when (and (has-key) (pos w28)) (key-at w28))  
            (when (and (has-key) (pos w29)) (key-at w29))  
            (when (and (has-key) (pos w30)) (key-at w30))  
            (when (and (not (locked w1)) (not (pos w1))) 
                  (oneof (closed w1) (not (closed w1))))
